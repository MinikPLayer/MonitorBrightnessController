...
using static winddcutil.HDRPInvoke;

namespace winddcutil
{
    public class HDRPInvoke
    {
        ...
        public static float GetHDRNits(DISPLAYCONFIG_PATH_INFO path)
        {
            ...
            sdrInfo.header.type = DISPLAYCONFIG_DEVICE_INFO_TYPE.DISPLAYCONFIG_DEVICE_INFO_GET_SDR_WHITE_LEVEL;
            sdrInfo.header.size = Marshal.SizeOf<DISPLAYCONFIG_SDR_WHITE_LEVEL>();
            sdrInfo.header.adapterId = path.targetInfo.adapterId;
            sdrInfo.header.id = path.targetInfo.id;
            var err = DisplayConfigGetDeviceInfo(ref sdrInfo);
            if (err != 0)
        ...
        public static bool IsHDREnabled()
        {
            ...
            foreach (var path in paths)
            {
                ...
                Debug.WriteLine(info.monitorFriendlyDeviceName);
                Debug.WriteLine(" Advanced Color Supported: " + colorInfo.advancedColorSupported);
                Debug.WriteLine(" Advanced Color Enabled  : " + colorInfo.advancedColorEnabled);
                Debug.WriteLine("");
